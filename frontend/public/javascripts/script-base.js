...
function menuEdit(id, form, endpoint) { // menu de edição, está sendo chamado por uma lista dinâmica
    ...
    fetch(url + endpoint + id)
        ...
        .then((data) => {
            if (Array.isArray(data)) { // se a resposta for um array, pega o primeiro [0]
                data = data[0];
            }
...
function telaInicial() { // exibe a tela inicial
    ...
    // removerRequired.forEach(item => removerAttrRequired(item)); TODO corrigir bug
...
function pegarTutor(e) { // não está sendo utilizado
    id = e.value.split('-')[0]
    carregar(`pets/`)
        .then((pets) => {
            lista = ``
